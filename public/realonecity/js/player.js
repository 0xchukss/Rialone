...
GTA.Player.prototype.attack = function (moveId) {
    if (this.game !== undefined && this.game.combat !== undefined &&
            this.game.combat.playerAttack !== undefined) {
        return this.game.combat.playerAttack(moveId || "shoot");
    }
...
GTA.Player.prototype.vehicleWillCrash = function (driveAngle, speed) {
    ...
    if (block === undefined || block.type === 0 || block.type === 1 || block.type === 5) {
        return true;
    }

    if (this.game.npcManager !== undefined && this.game.npcManager.getVehicleImpactForVehicle !== undefined) {
        impact = this.game.npcManager.getVehicleImpactForVehicle(this.vehicle, driveAngle, speed);

        if (impact !== null) {
            if (this.game.npcManager.resolveVehicleImpact !== undefined) {
                this.game.npcManager.resolveVehicleImpact(this.vehicle, impact);
            }

            if (impact.trafficCar !== undefined && this.game.npcManager.triggerTrafficDriverRetaliation !== undefined) {
                this.game.npcManager.triggerTrafficDriverRetaliation(impact.trafficCar);
            }
